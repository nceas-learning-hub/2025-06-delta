...
Perhaps you expected `mean_height_m` to change when you changed `weight_lb`?  It did not!  This demonstrates an important R programming concept: **Assigning a value to one object does not change the values of other objects in R.**  Until you recalculate the mean height, `mean_height_m` still has its original value.
